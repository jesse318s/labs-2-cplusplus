...
int swapColor(int c) { return (c == 0) ? 1 : 0; }
...
int RBTNode<T>::height() const {
	...
	if (left != nullptr) left_h = left->height();

	if (right != nullptr) right_h = right->height();
...
void RBTNode<T>::prettyPrint(int indent) const {
	...
	if (right != nullptr) { right->prettyPrint(indent + 1); }
	...
	if (left != nullptr) left->prettyPrint(indent + 1);
}

template <class T>
void swapColor(RBTNode<T>* node) {
	if (node != nullptr) node->color = swapColor(node->color);
}

// getColor handles null nodes
template <class T>
int getColor(RBTNode<T>* node) {
	if (node != nullptr) return node->color;
			...
			if (curr_node->parent == grandparent->left) {
				...
				if (getColor(uncle) == RED) {
					...
					if (curr_node == curr_node->parent->right) {
						// Case 2: curr_node is right child
						curr_node = curr_node->parent;
						singleCCR(curr_node);
					}

					// Case 3: curr_node is left child
					curr_node->parent->color = BLACK;
					grandparent->color = RED;
				...
				if (getColor(uncle) == RED) {
					...
					if (curr_node == curr_node->parent->left) {
						// Case 2: curr_node is left child
						curr_node = curr_node->parent;
						singleCR(curr_node);
					}

					// Case 3: curr_node is right child
					curr_node->parent->color = BLACK;
					grandparent->color = RED;
